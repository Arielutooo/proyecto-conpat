...
// ─── Admin Dashboard ──────────────────────────────────────────────────────────
const AdminDashboard = ({ clientes, onSelectCliente, onNewCliente }) => {
        ...
        <Card>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((c, idx) => {
                ...
                return (
                  <tr key={c.id} style={{ borderBottom: idx < filtered.length - 1 ? '1px solid #f8fafc' : 'none', transition: 'background 0.1s' }}
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <Btn size="sm" variant="secondary" icon="chevronRight" onClick={() => onSelectCliente(c)}>
                        Ver Panel
                      </Btn>
                    </td>
                  </tr>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8}><EmptyState icon="search" title="Sin resultados" sub="Intenta con otro término de búsqueda" /></td></tr>
              )}
            </tbody>
          </table>
        </Card>
