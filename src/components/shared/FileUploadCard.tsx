...
interface Props {
  ...
  canDefineVigencia?: boolean
  onDefineVigencia?: () => void
}

function VigenciaBadge({ validFrom, validUntil }: { validFrom: number | null | undefined; validUntil: number | null | undefined }) {
  if (!validUntil) return null
  const now = new Date().getFullYear()
  let bg: string, color: string, ring: string, label: string
  if (validUntil < now) {
    bg = '#fef2f2'; color = '#dc2626'; ring = '#fecaca'; label = `Vencido ${validUntil}`
  } else if (validUntil === now) {
    bg = '#fffbeb'; color = '#d97706'; ring = '#fde68a'; label = `Vence ${validUntil}`
  } else {
    bg = '#f0fdf4'; color = '#15803d'; ring = '#bbf7d0'
    label = validFrom ? `${validFrom}–${validUntil}` : `Hasta ${validUntil}`
  }
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: bg, color, border: `1px solid ${ring}`, borderRadius: 20, padding: '2px 8px', fontSize: 10, fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0 }}>
      {label}
    </span>
  )
}
...
export function FileUploadCard({
  ...
  validFrom, validUntil, canDefineVigencia, onDefineVigencia,
}: Props) {
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          ...
          {hasFile && <VigenciaBadge validFrom={validFrom} validUntil={validUntil} />}
        </div>
              ...
              {menuOpen && (
                <>
                  ...
                  <div style={{
                    ...
                    <button
                      onClick={() => { setMenuOpen(false); onDefineVigencia?.() }}
