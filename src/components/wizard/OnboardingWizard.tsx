...
import { TopBar, Card, Btn, Input, Select } from '@/components/shared'
import { Icon } from '@/components/shared/Icon'
import { useApp } from '@/lib/context'
import { TIPO_INV_LABELS } from '@/lib/helpers'
...
const TRow = ({ label, sub, value, setter, icon }: { label: string; sub?: string; value: boolean; setter: (v: boolean) => void; icon: string }) => (
  <div
    onClick={() => setter(!value)}
    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${value ? 'oklch(0.55 0.18 245)' : '#e5e8ef'}`, background: value ? 'oklch(0.97 0.04 245)' : 'white', cursor: 'pointer', transition: 'all 0.15s', marginBottom: 8 }}
  >
    <div style={{ width: 34, height: 34, borderRadius: 8, background: value ? 'oklch(0.55 0.18 245)' : '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
    ...
    <div style={{ width: 20, height: 20, borderRadius: 4, border: `2px solid ${value ? 'oklch(0.55 0.18 245)' : '#d1d5db'}`, background: value ? 'oklch(0.55 0.18 245)' : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
...
interface OnboardingWizardProps {
  onFinish: (c: Cliente) => void
  onCancel: () => void
}

export const OnboardingWizard = ({ onFinish, onCancel }: OnboardingWizardProps) => {
  ...
  const updateSocio = (i: number, k: string, v: string) => { const a = [...socios]; a[i] = { ...a[i], [k]: v }; setSocios(a) }
  const addSocio = () => setSocios([...socios, emptySocio()])
  const removeSocio = (i: number) => socios.length > 1 && setSocios(socios.filter((_, idx) => idx !== i))
  const totalPct = socios.reduce((s, x) => s + (parseFloat(x.porcentaje_participacion) || 0), 0)
  const pctOk = Math.abs(totalPct - 100) < 0.01

  const updateInv = (i: number, k: string, v: string) => { const a = [...inversiones]; a[i] = { ...a[i], [k]: v }; setInversiones(a) }
  const addInv = () => setInversiones([...inversiones, emptyInversion()])
  const removeInv = (i: number) => inversiones.length > 1 && setInversiones(inversiones.filter((_, idx) => idx !== i))
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
      const newCliente: Cliente = {
        ...
        rut: rut.trim(),
        tipo_sociedad: tipo_sociedad as Cliente['tipo_sociedad'],
        regimen_tributario,
        actividad_economica: actividad_economica.trim(),
        codigo_sii: codigo_sii.trim(),
        iniciacion_actividades,
        rentas_presuntas,
        factura_conpat,
        factura_moneda,
        factura_monto,
        ...
        inversiones: sinInversiones ? [] : inversiones.map((inv, i) => ({
          ...inv,
          id: `ni${i}`,
          tipo_inversion: inv.tipo_inversion as Cliente['inversiones'][0]['tipo_inversion'],
          ingreso_mensual_asociado: 0,
          aum_apertura: parseFloat(inv.aum_apertura) || 0,
          aum_cierre: parseFloat(inv.aum_cierre) || 0,
        })),
        retiros: [],
        cartolas: [],
        entregables: [],
          ...
          {STEPS.map((s, idx) => {
            ...
            return (
              <div key={s.n} style={{ display: 'flex', gap: 14, marginBottom: 32, position: 'relative' }}>
                ...
                <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: done ? 'oklch(0.55 0.18 245)' : active ? 'white' : '#f8fafc', border: `2px solid ${done ? 'oklch(0.55 0.18 245)' : active ? 'oklch(0.55 0.18 245)' : '#e5e8ef'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                  {done ? <Icon name="check" size={12} style={{ color: 'white' }} /> : <span style={{ fontSize: 11, fontWeight: 700, color: active ? 'oklch(0.55 0.18 245)' : '#94a3b8' }}>{s.n}</span>}
                </div>
                <div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 36 }}>
            ...
            <div style={{ maxWidth: 640 }}>
              ...
              <Card style={{ padding: 24, marginBottom: 18 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                  ...
                  <Select label="Tipo de Sociedad" value={tipo_sociedad} onChange={e => setTipoSociedad(e.target.value)} required options={['SpA', 'Ltda', 'EIRL', 'SA', 'EU'].map(v => ({ value: v, label: v === 'EU' ? 'EU — Empresa Unipersonal' : v }))} />
                  <Select label="Régimen Tributario" value={regimen_tributario} onChange={e => setRegimen(e.target.value)} required options={['14A Semi-Integrado', '14D Transparente', '14 TER Pyme', '14D Pyme'].map(v => ({ value: v, label: v }))} />
                </div>
              ...
              <Card style={{ padding: 24, marginBottom: 18 }}>
                ...
                {factura_conpat && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 4 }}>
                    <Select label="Moneda" value={factura_moneda} onChange={e => setFacturaMoneda(e.target.value)} options={['CLP', 'UF', 'USD'].map(v => ({ value: v, label: v }))} />
              ...
              <Card style={{ padding: 24 }}>
                ...
                {socios.map((s, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px 32px', gap: 12, marginBottom: 12, alignItems: 'end' }}>
                    ...
                    <button onClick={() => removeSocio(i)} disabled={socios.length === 1} style={{ height: 36, width: 32, border: '1px solid #fecaca', borderRadius: 8, background: '#fef2f2', color: '#dc2626', cursor: socios.length === 1 ? 'not-allowed' : 'pointer', opacity: socios.length === 1 ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Icon name="x" size={12} />
                    </button>
                  </div>
                ))}
          ...
          {step === 2 && (
            <div style={{ maxWidth: 640 }}>
              ...
              <div
                onClick={() => setSinInversiones(!sinInversiones)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: `1.5px solid ${sinInversiones ? 'oklch(0.55 0.18 245)' : '#e2e8f0'}`, borderRadius: 12, background: sinInversiones ? 'oklch(0.97 0.04 245)' : 'white', cursor: 'pointer', transition: 'all .2s', marginBottom: 20 }}
              >
                <div>
                  ...
                  {inversiones.map((inv, i) => (
                    <Card key={i} style={{ padding: 20, marginBottom: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                        ...
                        {inversiones.length > 1 && <button onClick={() => removeInv(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: 11 }}>Eliminar</button>}
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 12 }}>
                        <Select label="Tipo de Inversión" value={inv.tipo_inversion} onChange={e => updateInv(i, 'tipo_inversion', e.target.value)} options={Object.entries(TIPO_INV_LABELS).map(([v, l]) => ({ value: v, label: l }))} />
                        <div />
            ...
            <div style={{ maxWidth: 640 }}>
              ...
              <Card style={{ padding: 24, marginBottom: 24 }}>
                ...
                {[
                  ...
                  <div key={opt.key} onClick={() => opt.setter(!opt.value)} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', borderRadius: 10, border: `1.5px solid ${opt.value ? 'oklch(0.55 0.18 245)' : '#e5e8ef'}`, background: opt.value ? 'oklch(0.97 0.04 245)' : 'white', cursor: 'pointer', marginBottom: 10, transition: 'all 0.15s' }}>
                    ...
                    <div style={{ width: 20, height: 20, borderRadius: 4, border: `2px solid ${opt.value ? 'oklch(0.55 0.18 245)' : '#d1d5db'}`, background: opt.value ? 'oklch(0.55 0.18 245)' : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
                  {[
                    ...
                    <div key={String(k)}>
                      ...
                      <span style={{ fontWeight: 600, color: '#374151' }}>{String(v)}</span>
                    </div>
                  ))}
                </div>
              </Card>
