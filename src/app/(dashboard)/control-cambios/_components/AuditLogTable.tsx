...
export function AuditLogTable({ logs, entityTypes }: Props) {
  ...
  }, [logs, entityFilter, search, dateFrom, dateTo])
  ...
  return (
    <div>
      ...
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e8ef', padding: '16px 20px', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        {(search || entityFilter !== 'todos' || dateFrom || dateTo) && (
          <button
            onClick={() => { setSearch(''); setEntityFilter('todos'); setDateFrom(''); setDateTo('') }}
            style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'white', fontSize: 12, color: '#64748b', cursor: 'pointer', alignSelf: 'flex-end' }}
          >
            Limpiar filtros
          </button>
        )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                {['Fecha y hora', 'Usuario', 'Descripción', 'Entidad'].map(h => (
              ...
              {filtered.map((log, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#475569', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {log.user_email}
                    </td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 9px', borderRadius: 99, fontSize: 11, fontWeight: 500, background: ec.bg, color: ec.text }}>
