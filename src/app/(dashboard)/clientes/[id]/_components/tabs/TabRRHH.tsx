...
export function TabRRHH({ cliente, role, anioFiscal }: Props) {
  const [tieneNomina, setTieneNomina] = useState(cliente.tiene_nomina)
  ...
  const toggleNomina = () => {
    const next = !tieneNomina
    setTieneNomina(next)
    startTransition(async () => {
      await updateCliente(cliente.id, { tiene_nomina: next })
    })
  }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-center justify-between shadow-sm">
        <div>
          <h3 className="font-bold text-[14px] text-slate-900">Maneja nómina de trabajadores</h3>
          <p className="text-[12px] text-slate-500 mt-0.5">Activa para gestionar contratos, liquidaciones y documentos laborales</p>
        </div>
        <div
          onClick={canEdit ? toggleNomina : undefined}
          className={`w-12 h-7 rounded-full relative transition-colors ${canEdit ? 'cursor-pointer' : ''} ${tieneNomina ? 'bg-blue-600' : 'bg-slate-200'}`}
        >
          <div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all shadow ${tieneNomina ? 'left-6' : 'left-1'}`} />
        </div>
      </div>
      ...
      {!tieneNomina ? (
        <div className="bg-white rounded-xl border border-slate-200 p-10 text-center shadow-sm">
          ...
          <p className="text-[13px] text-slate-400">Activa el toggle de arriba para gestionar documentos laborales.</p>
        </div>
      ) : (
        <>
          <div className="mb-6">
